...
<%
	...
	Connection con=null;
     //형식) jdbc:oracle:thin:@상대방의 접속ip주소:접속포트번호:SID=>xe버전 설치시 xe
     String url="jdbc:oracle:thin:@localhost:1521:orcl2";//접속 오라클의 위치
     
     Statement stmt=null;//create table
     PreparedStatement pstmt=null;//insert into->실행속도가 더 빠르다.
     ...
     try {
   	  ...
   	  System.out.println("테이블 성공유무(create)=>"+create);//create만 =>0, 나머지는 1일때 성공
   	  
   	  //4.insert 
   	  //sql="insert into Mytest values(?,?)"; ?는 집어넣을 자리.
   	  ...
   	  System.out.println("데이터가 입력성공유무(insert)="+insert);//create만 =>0, 나머지는 1일때 성공
   	  //5.select->필드별로 출력해서 결과 보기
   	  stmt2=con.createStatement();//객체 얻어와야함.
   ...
   <%
   	   ...
   	   rs.close();stmt2.close();pstmt.close();
