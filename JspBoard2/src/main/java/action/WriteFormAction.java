...
public class WriteFormAction implements CommandAction {
	...
	public String requestPro(HttpServletRequest request, HttpServletResponse response) throws Throwable {
		...
		//1. writeForm.jsp에서 처리하는 자바코드 이전

		   //list.do(글쓰기)->신규글,content.do(글상세보기->글쓰기(답변글))
		   ...
		   request.setAttribute("re_step", re_step);//${re_step}
		   request.setAttribute("re_level", re_level);//${re_level}
