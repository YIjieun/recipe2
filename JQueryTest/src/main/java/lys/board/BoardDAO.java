...
public class BoardDAO {//MemberDAO
	...
	//-----검색어에 해당되는 총레코드수를 구하는 메서드(search(검색분야), searchtext(검색어))----------------------------------------
	public int getArticleSearchCount(String search, String searchtext) {//getmemberCount()->MemberDAO에서 작성
		...
		try {
			...
			System.out.println("con=>"+con);
			//-----------------------------------
			if(search==null || search=="") {//검색분야를 선택하지 않았다면
				sql="select count(*) from board";
	...
	//3. 페이징 처리 계산 정리해주는 메서드(ListAction)+list.jsp
	//Hashtable=> 페이징 처리에 관련된 처리결과를 저장할 하나의 객체에 담아서 반환
	public Hashtable pageList(String pageNum, int count) {
	  ...
	  //String pageNum=request.getParameter("pageNum");=>ListAction에서 넘어온다.
	  if(pageNum==null){
	  ...
	  //모델1의 list.jsp 페이징 처리 복사
		...
		//페이징 처리에 대한 계산결과->Hashtable or HashMap=>ListAction에 전달->list.jsp
	...
	public void insertArticle(BoardDTO article) {//~(MemberDTO mem)
		...
		try {
			...
			if(num!=0) {
			   ...
			   re_step=re_step+1;
			   re_level=re_level+1;
			   System.out.println("답변글에서의 re_level계산 결과값=>"+re_level);
			}else {//신규글이라면 num=0
	...
	//실시간 검색어->writer(회원id 대신으로 사용)
	//select writer from board where writer like '%검색어%';//writer 검색
